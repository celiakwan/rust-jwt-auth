    ...
    fn from_request(request: &'a Request<'r>) -> Outcome<Self, ()> {
        ...
        match read_token(token_strs[0]) {
            Ok(api_key) => {
                let path = request
                    .raw_segment_str(0)
                    .expect("Failed to extract the path segment");
                match path.as_str() {
...
fn read_token(token_str: &str) -> Result<ApiKey, Error> {
    ...
    match verified_token {
        Ok(token) => Ok(ApiKey {
            ...
            role: Role::from_str(&token.claims()["role"])
                .expect("Failed to convert string to Role"),
...
fn authorize_with_role(role: Role, api_key: ApiKey) -> Outcome<ApiKey, ()> {
    let is_authorized =
        api_key.role == role && api_key.exp > Utc::now().timestamp() && is_logged_in(&api_key);
...
fn is_logged_in(api_key: &ApiKey) -> bool {
    User::get_by_username(&api_key.sub)
        .expect(&format!(
            "Failed to find the user by username: {}",
            api_key.sub
        ))
        .logged_in
}
